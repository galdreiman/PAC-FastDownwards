...
static SearchEngine *_parse_anytime_weighted_astar(OptionParser &parser) {
    ...
    if (!parser.dry_run()) {
        ...
        engine->InitPacVariables(pac_epsilon,pac_delta);
...
void LazySearch::InitPacVariables(double epsilon, double delta)
{
    
    cout << "INFO: InitPacVariables" << std::endl;

     m_pac_epsilon = epsilon;
     delta = 100 - (int)(delta * 100);
     bool found = false;
     ...
     cout << "pac_probability: " << delta << std::endl;
