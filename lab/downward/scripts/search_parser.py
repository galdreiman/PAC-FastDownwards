...
ITERATIVE_PATTERNS = PORTFOLIO_PATTERNS + [
    ...
    ('initial_ff_h_value', re.compile(r'Initial ff_heuristic: (\d+)\.'), int),
...
def get_initial_ff_h_value(content, props):
    # Ignore logs from searches with multiple heuristics.
    if 'initial_ff_h_value' not in props:
        pattern = r'^Initial ff_heuristic: (\d+)'
        match = re.search(pattern, content, flags=re.M)
        if match:
            props['initial_ff_h_value'] = int(match.group(1))
...
class SingleSearchParser(SearchParser):
    def __init__(self):
        ...
        self.add_function(get_initial_ff_h_value)
