...
int StandardScalarOpenList<Entry>::insert(const Entry &entry) {
    ...
    // cout << "last_evaluated_value = [" << last_evaluated_value << "]" <<endl; 
...
template<class Entry>
StandardScalarOpenListForAnyTime<Entry>::StandardScalarOpenListForAnyTime(
    ScalarEvaluator *eval, ScalarEvaluator *eval_for_anytime, bool preferred_only)
    : StandardScalarOpenList<Entry>(eval, preferred_only), anytime_evaluator(eval_for_anytime) {
}

template<class Entry>
int StandardScalarOpenListForAnyTime<Entry>::evaluate_for_anytime(int g, bool preferred) {
    get_anytime_evaluator()->evaluate(g, preferred);
    return get_anytime_evaluator()->get_value();
}

template<class Entry>
void StandardScalarOpenList<Entry>::getFlattedOpenList(std::vector<Entry> &res)
{
    ...
    int in;
    cin >> in;
    typename std::map<int, Bucket>::iterator bucketItr;
    for(bucketItr = buckets.begin(); bucketItr != buckets.end(); ++ bucketItr)
    {
        // cout << "buckets size =[" << bucketItr->second.size() << "]" << endl;
        // cout << "buckets first =[" << bucketItr->first << "]" << endl;
        typename Bucket::iterator dequeItr;
        for(dequeItr = (*bucketItr).second.begin(); dequeItr != (*bucketItr).second.end(); ++dequeItr)
        {
           res.push_back(*dequeItr);
        }
    }
